//
//  LPNewDefaultEffectViewController.m
...
@interface LPNewDefaultEffectViewController ()
...
@implementation LPNewDefaultEffectViewController
...
- (void)viewDidLoad
{
    ...
    self.title = [[[LPSharedManager sharedManager] languageDictionary] objectForKey:@"NewDefaultEffectViewTitle"];
    ...
    self.repeatsLabel.text = [[[LPSharedManager sharedManager] languageDictionary] objectForKey:@"EffectRepeatsLabelText"];
    ...
    self.repeatsSlider.value = 10;
    [self repeatsSliderValueChanged:nil];
...
- (void)saveButtonClicked
{
    ...
    if (self.nameTextField.text.length > 0) {
        ...
        if (self.function == 0) {
            NSMutableArray *array = [LPAnimations getArrayWavesDelay:self.delaySlider.value delayUnit:self.selectedDelayUnit repeats:self.repeatsSlider.value effectName:self.nameTextField.text];
            
            [effects addObjectsFromArray:array];
        } else if (self.function == 1) {
            NSMutableArray *array = [LPAnimations getArrayCubes4Delay:self.delaySlider.value delayUnit:self.selectedDelayUnit repeats:self.repeatsSlider.value effectName:self.nameTextField.text];
...
{
    int repeats = (int)self.repeatsSlider.value;
    
    self.repeatsValueLabel.text = [NSString stringWithFormat:@"%d", repeats];
}
